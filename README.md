...
Os **assets Flash 4.1 do jogo já estão incluídos** no repositório: SWFs, XMLs, imagens de UI e arquivos de runtime do cliente. O único componente grande que continua fora do git são os backups `.bak` do SQL Server.
...
*   `Client/`: Arquivos que devem ser hospedados no IIS (porta 8080).
    ...
    *    **Flash:** Os arquivos nativos Flash (SWF), UI, XMLs, imagens e resources servidos pelo navegador. O pacote 4.1 já inclui `ui/vietnam`, `Launcher.swf`, `Loading.swf`, `shape.swf` e demais assets necessários.
...
5. Os assets Flash 4.1 já estão incluídos no diretório `Client/Flash`, então não é mais necessário buscar os SWFs separadamente.
...
O repositório agora já inclui o pacote principal do cliente Flash 4.1:

- `Client/Flash/ui/vietnam/swf/` com os módulos de interface do jogo
- `Client/Flash/ui/vietnam/xml/` com as definições de UI
- `Client/Flash/ui/vietnam/img/` com imagens de suporte
- assets raiz como `Launcher.swf`, `Loading.swf`, `DDT_Loading.swf`, `audio.swf`, `shape.swf` e `tutorial.swf`

Isso permite subir o cliente web sem depender de um pacote Flash externo adicional.
